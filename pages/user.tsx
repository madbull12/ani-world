...
const Favourite = ({ item }: { item: IFavourite }) => {
    ...
    const deleteSave = async(id:number) => {
        try {
            await toast.promise(fetch(`/api/favorite/${id}`, {
                headers:{
    ...
    return (
        <Link href={`/anime/${item.malId}`} key={uuidv4()}  >
            <div className='flex flex-col font-bold cursor-pointer relative space-y-2' onMouseEnter={()=>setShowTrash(true)} onMouseLeave={()=>setShowTrash(false)}>
                <Image src={item.imageUrl} height={300} width={250} className="rounded-lg " objectFit={"cover"} alt={item.title} />
                <p>{item.title}</p>
                {showTrash && (
                    <motion.button  className='text-gray-300 self-center absolute left-2' onClick={(e)=>{
                        e.stopPropagation()
                        deleteSave(item.id)
                    }}>
